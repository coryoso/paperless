...
function setup() {
  ...
  return { page, browser, refresh, onUploads, streams, connect, stop };
}
...
test("multiplexes upload batches on the dashboard stream and removes handlers on close", () => {
  const { streams, connect, refresh, onUploads, stop } = setup();
  const snapshots = Array.from({ length: 10 }, (_, i) => ({
    run_id: `run-${i}`,
    client_upload_id: `client-${i}`,
    events: [{ done: false }],
  }));
  const emit = () =>
    streams[0].dispatchEvent(
      new MessageEvent("uploads", { data: JSON.stringify(snapshots) }),
    );
  emit();
  expect(onUploads).toHaveBeenCalledWith(snapshots);
  expect(refresh).not.toHaveBeenCalled();
  snapshots[0].events[0].done = true;
  emit();
  expect(refresh).toHaveBeenCalledTimes(1);
  expect(connect).toHaveBeenCalledTimes(1);
  stop();
  emit();
  expect(onUploads).toHaveBeenCalledTimes(2);
});
...
test("retries a failed snapshot even without another SSE event", async () => {
  ...
  const fetch = mock(async () => {
    if (fetch.mock.calls.length === 1) throw new Error("Offline");
    return 2;
  });
  const accept = mock((_snapshot: number) => {});
  const failed = mock(() => {});
...
test("aborts requests and suppresses late results after disposal", async () => {
  ...
  const fetch = mock((s: AbortSignal) => {
    signal = s;
