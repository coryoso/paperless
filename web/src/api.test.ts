...
it("shows a readable setup error when the selected model is unavailable", async () => {
  globalThis.fetch = mock(() => Promise.resolve(new Response(JSON.stringify({ error: "Start Ollama before continuing." }), { status: 400 }))) as unknown as typeof fetch;
  await expect(api.setModelProvider("ollama", true)).rejects.toThrow(/^Start Ollama before continuing\.$/);
});
...
describe("dashboard API", () => {
  ...
  it("loads the database-backed dashboard", async () => {
    const payload: Dashboard = {
      settings: { inbox: "/inbox", archive_root: "/archive", archive_exists: true, archive_error: "", setup_required: false, setup_step: "complete", scanner_share_checked: true, scanner_share_ready: true, model: "qwen3.5", model_provider: "ollama", model_enabled: true },
  ...
  it("starts the native documents-directory chooser", async () => {
    ...
    const fetchMock = mock(() => Promise.resolve(new Response(JSON.stringify(payload), { status: 202 })));
    globalThis.fetch = fetchMock as unknown as typeof fetch;

    await expect(api.chooseDocumentsDirectory()).resolves.toEqual(payload);
    expect(fetch).toHaveBeenCalledWith("/api/setup/documents-directory", { method: "POST" });
  });
...
describe("Bonsai setup", () => {
  it("saves Bonsai as the provider", async () => {
    globalThis.fetch = mock(() => Promise.resolve(new Response(JSON.stringify({ provider: "bonsai", restarting: true }), { status: 202 }))) as unknown as typeof fetch;
    await expect(api.setModelProvider("bonsai")).resolves.toEqual({ provider: "bonsai", restarting: true });
  });
  it("reads installation updates split across network chunks", async () => {
    const chunks = ['{"message":"Down', 'loading…"}\n{"message":"Ready"}\n', '{"done":true}\n'];
    ...
    globalThis.fetch = mock(() => Promise.resolve(new Response(stream))) as unknown as typeof fetch;
    const updates: string[] = [];
    await api.installBonsai((message) => updates.push(message));
    expect(updates).toEqual(["Downloading…", "Ready"]);
    expect(fetch).toHaveBeenCalledWith("/api/setup/bonsai/install", { method: "POST", headers: { "Content-Type": "application/json" }, body: '{"install":true}' });
  });
  it("reports installation failures and premature disconnects", async () => {
    for (const [body, error] of [['{"error":"Download failed"}\n', "Download failed"], ['{"message":"Downloading"}\n', "connection closed"]]) {
  ...
  it("reports rejected installation requests", async () => {
    globalThis.fetch = mock(() => Promise.resolve(new Response("Already installing", { status: 409 }))) as unknown as typeof fetch;
    await expect(api.installBonsai(() => {})).rejects.toThrow("Already installing");
  });
});

describe("first-run guide", () => {
  it("saves the choice to continue without AI", async () => {
    globalThis.fetch = mock(() => Promise.resolve(new Response(JSON.stringify({ provider: "ollama", restarting: true }), { status: 202 }))) as unknown as typeof fetch;
    await api.setModelProvider("ollama", false);
    expect(fetch).toHaveBeenCalledWith("/api/setup/model", { method: "POST", headers: { "Content-Type": "application/json" }, body: '{"provider":"ollama","enabled":false}' });
  });
  it("records progress and explicit completion", async () => {
    globalThis.fetch = mock(() => Promise.resolve(new Response(JSON.stringify({ restarting: true }), { status: 202 }))) as unknown as typeof fetch;
    await api.setupProgress("model");
    expect(fetch).toHaveBeenLastCalledWith("/api/setup/progress", { method: "POST", headers: { "Content-Type": "application/json" }, body: '{"step":"model"}' });
    await api.setupProgress("complete");
    expect(fetch).toHaveBeenLastCalledWith("/api/setup/progress", { method: "POST", headers: { "Content-Type": "application/json" }, body: '{"step":"complete"}' });
  });
});
