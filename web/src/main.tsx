  ...
  return (
    ...
    >
              ...
              {!dashboard.settings.setup_required && (
                <>
                  ...
                  <NavButton
                    ...
                    icon={
                      <LoaderCircle className={processingCount ? "spin" : ""} />
                    }
                    label="Processing"
      ...
      <main className="main">
        ...
        {!loading && dashboard.settings.setup_required && (
          <SetupGuide
            dashboard={dashboard}
            onRefresh={load}
...
function ProcessingWorkspace({
  ...
  const entries = [
    ...uploads.map((task) => ({
      id: task.id,
      task,
      job: allJobs.find((job) => job.id === task.jobID),
    })),
    ...jobs
      .filter((job) => !represented.has(job.id))
      .map((job) => ({ id: job.id, task: undefined, job })),
  ];
  const selected =
    entries.find((entry) => entry.id === selectedID) ?? entries[0];
  const active = entries.filter((entry) =>
    entry.task
      ? entry.task.state === "uploading" || entry.task.state === "processing"
      : true,
  ).length;
            ...
            {entries.map((entry) => {
              ...
              return (
                <button
                  type="button"
                  key={entry.id}
                  className={`job-row queue-row${selected?.id === entry.id ? " selected" : ""}`}
...
function ProcessingDetail({
  ...
  return (
    <article className="processing-detail">
      <header className="processing-detail-head">
        ...
        <div>
          <span className="eyebrow">{processingEntryLabel(task, job)}</span>
          <h2>{task?.filename || job?.source_filename}</h2>
          <p>{latest?.message || processingStatusMessage(job?.status)}</p>
        </div>
      ...
      {(task?.error || job?.error) && (
        <div className="inline-error">
          <CircleAlert /> {task?.error || job?.error}
        </div>
      )}
      <div className="fact-row queue-facts">
        ...
        <Fact
          label="Current stage"
          value={displayName(latest?.phase || job?.status || "upload")}
        />
      </div>
      <section className="processing-events">
        ...
        {task?.events.length ? (
          <div className="queue-timeline">
            {task.events.map((event, index) => (
              ...
              >
                ...
                <div>
                  <strong>{displayName(event.phase)}</strong>
                  <p>{event.message}</p>
                </div>
              </div>
            ))}
          ...
          <p className="processing-note">
            This document was already processing when the page was opened. Its
...
function processingEntryLabel(task?: UploadTask, job?: Job): string {
  ...
  if (task?.state === "complete")
    return job?.status === "needs_review" ? "Ready for review" : "Complete";
...
function processingEntryPercent(task?: UploadTask, job?: Job): number {
  ...
  return (
    ...
    (job && !isProcessingStatus(job.status) ? 100 : 0)
  );
}
  ...
  return (
    <>
      <section className="stats-grid">
        ...
        <Stat
          icon={<FileCheck2 />}
          value={dashboard.stats.archived}
          label="Archived"
        />
        <Stat
          icon={<Files />}
        ...
        <Stat
          icon={<FolderArchive />}
          value={dashboard.folders.length}
          label="Known folders"
        />
      </section>
      <section className="overview-grid">
        <div>
          <SectionHead
            title="Review queue"
            meta={`${dashboard.review_jobs.length} waiting`}
          />
          <JobList
            jobs={dashboard.review_jobs.slice(0, 4)}
...
function Stat({
  ...
  return (
    <button
      type="button"
      className={accent ? "stat-tile accent" : "stat-tile"}
      onClick={onClick}
      disabled={!onClick}
    >
      <span>{icon}</span>
    ...
    <section className="workspace-grid">
      ...
      <div className="detail-column">
        {selected ? (
          ...
          <EmptyState icon={<FileCheck2 />} title="Review complete" />
        )}
      </div>
    </section>
  );
  ...
  return (
    <section className="documents-workspace">
      <div className="documents-head">
        <SectionHead title="All documents" meta={`${visible.length} shown`} />
    ...
    <div className="job-list">
      {jobs.map((job) => (
        <button
          type="button"
          key={job.id}
          className={`${compact ? "job-row compact" : "job-row"}${selectedID === job.id ? " selected" : ""}`}
        ...
        >
          ...
          <div className="job-side">
            ...
            {job.confidence ? <b>{Math.round(job.confidence * 100)}%</b> : null}
            <ChevronRight />
          </div>
        </button>
      ))}
...
function JobDetail({
  ...
  const reject = async () => {
    if (
      !window.confirm(
        "Reject and permanently delete this document and its stored copies?",
      )
    )
      return;
    setSaving(true);
      ...
      <div className="preview-toolbar">
        <div className="segmented" role="tablist">
          <button
            ...
            onClick={() => setMode("pdf")}
          >
            {job.text_source === "embedded" ? "Original PDF" : "OCR PDF"}
          </button>
          {job.text_source === "ocr" && (
            <button
              type="button"
